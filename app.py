...
def answer_question(file,question):
  ...
  if not question.strip():
    return "Please enter a question first.",None
  uploaded_file_ref=client.files.upload(file=file.name)
  response = client.models.generate_content(
      model="gemini-2.5-flash",
      contents=[
          uploaded_file_ref,
          f"Answer this question using ONLY this document.\n\nQuestion: {question}"
      ]
  )
  return response.text,None
...
#UI layout(No Dropdown Lauyout)
with gr.Blocks()as demo:
  gr.Markdown("#  Gemini Document Summarizer")
  with gr.Row():
    ...
    with gr.Column(scale=2):
      ...
      with gr.Row():
        ...
        btn_audio =gr.Button("Audio Summary",variant="secondary")
      gr.Markdown("---")
